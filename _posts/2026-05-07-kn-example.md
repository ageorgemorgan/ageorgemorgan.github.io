...
> **Definition** 
    <ul>
        <li>
            ...
            \begin{equation}
                \label{eqn:cbnorm}
                \left\|f\right\|_{C_{b}\left(\mathbb{R}^3\right)} \doteq \sup_{x\in \mathbb{R}^3}\left|f(x)\right|. 
                \end{equation}
        </li>
        <li>
            ...
            \begin{equation}
                \label{eqn:cbnorm_vect}
                \left\|\mathbf{V}\right\|_{C_{b}\left(\mathbb{R}^3\right)} \doteq \sup_{x\in \mathbb{R}^3}\left|\mathbf{V}(x)\right|, 
                \end{equation}
            where $\left|\mathbf{V}(x)\right|$ is the Euclidean norm of $\mathbf{V}(x)$. 
        </li>
        <li>
            We define $C^{1}_{b}\left(\mathbb{R}^3\right)$ to be the subspace of $C_{b}\left(\mathbb{R}^3\right)$ consisting of those bounded continuous functions $f(x)$ whose gradients $\nabla f(x)$ have entries in $C_{b}\left(\mathbb{R}^3\right)$. We put another norm on this space by defining 
            \begin{equation}
                \label{eqn:cb1norm}
                \left\|f\right\|_{C^{1}_{b}\left(\mathbb{R}^3\right)} \doteq  \left\|f\right\|_{C_{b}\left(\mathbb{R}^3\right)}+ \left\|\nabla f\right\|_{C_{b}\left(\mathbb{R}^3\right)} . 
            \end{equation}
        </li>
    </ul>
...
<ul>
    <li> 
        <b>Case 1: $|t|\geq 1$.</b> By hypothesis and the Kirchhoff formula, $u(x,t)$ depends only on the values of $F$ and $G$ on $\partial B(x,|t|) \cap B(0,R)$: $F,G$ vanish at all other points in $\partial B(x,|t|)$. Now, notice that the largest sphere that can be inscribed inside the closure $\overline{B(0,R)}$ is simply $\partial B(0,R)$. Therefore, 
        $$
        ...
        \begin{align*}
            |u(x,t)| &\leq \frac{1}{4\pi t^2} \oint_{\partial B(x,|t|) \cap B(0,R)} |F(y)| + |\nabla F(y)| \ |y-x| + |t| \ |G(y)| \ \diff S(y)
            \\
            &\leq \frac{1}{4\pi t^2} \ \left(\left\|F\right\|_{C_{b}}+|t| \ \left\|\nabla F\right\|_{C_{b}} + |t|\ \left\|G\right\|_{C_{b}}\right)\oint_{\partial B(x,|t|) \cap B(0,R)}\diff S(y)
            \\
    ...
    <li>
        <b>Case 2: $|t|< 1$.</b>  As in the previous case, we apply the Kirchhoff formula to estimate 
        \begin{align}
            |u(x,t)| &\leq \frac{1}{4\pi t^2} \oint_{\partial B(x,|t|)} |F(y)| + |\nabla F(y)| \ |y-x| + |t| \ |G(y)| \ \diff S(y) \nonumber
            \\
            &\leq \left\|F\right\|_{C_{b}}\frac{1}{4\pi |t|^2} \oint_{\partial B(x,|t|)} \diff S(y) + \left(\left\|\nabla F\right\|_{C_{b}}+\left\| G\right\|_{C_{b}}\right) \frac{1}{4\pi |t|} \oint_{\partial B(x,|t|)} \diff S(y) \nonumber
            \\
...
\par Let's pretend we have found a $v(x,t)$ solving \eqref{eqn:nlw_kn}. What IVP $u(x,t)$ does satisfy? A direct computation shows that 
\begin{align*}
    u_{t} &= v_{t}e^{v}
    \\
...
\begin{subequations}\label{eqn:ICs_nlw}
\begin{align}
    u|_{t=0} &= F(x) \doteq e^{f(x)}-1, 
    \\
    u_{t}|_{t=0} &= G(x) \doteq g(x)e^{f(x)}. 
\end{align}
\end{subequations}
...
\begin{enumerate}
    \item $u(x,t)$ solves \eqref{eqn:wave_ivp} with the particular $F, G$ defined in \eqref{eqn:ICs_nlw} and
    \item $\|u(\cdot, t)\|_{C_{b}\left(\mathbb{R}^3\right)}<1$,
\end{enumerate}  then $v$ defined by \eqref{eqn:u_to_v} solves \eqref{eqn:nlw_kn}. This gives us a recipe for constructing $v(x,t)$ explicitly by solving the linear wave equation and then bounding its solution!
\par So, let's define $u$ to be the solution of \eqref{eqn:wave_ivp} guaranteed to exist (and decay in time) by proposition \ref{prop:wave_eqn_decay}. We now split into two cases depending on the size of $|t|$. 
<ul>
    <li> <b>Case 1: $|t|\geq 1$.</b> In this case we see straight from \eqref{eqn:wave_eqn_decay} that $\|u(\cdot, t)\|_{C_{b}\left(\mathbb{R}^3\right)}<1$ provided
    \begin{equation}
    ...
    <li> <b>Case 2: $|t|< 1$.</b> Here we can't use \eqref{eqn:wave_eqn_decay} directly. Instead, we go back to the intermediary result \eqref{eqn:important_intermediate} in the proof of proposition \ref{prop:wave_eqn_decay}, which tells us that $\|u(\cdot, t)\|_{C_{b}\left(\mathbb{R}^3\right)}<1$ provided
    $$
